...
/**
 * Controller for screen where manager can set interest rates for accounts
 */
public class ManagerInterestController extends Controller {
  ...
  /**
   * Button that lets manager set interest rates
   */
  @FXML
  void interestClicked(ActionEvent event) {

    try {
      //check that entered info is formatted properly
      double newInterest = Double.parseDouble(interestRateField.getText());
      String accountID = accountField.getText();

      //check if accountID is a valid savings account (not CD!)
      if (accountID.length() == 11 && getLoginController().hasValidSavingsAccount(accountID)) {
        ...
        if (
          ...
          //update that checking interest too
        ...
        a.setContentText("Please enter a valid savings account number (not CD).");
