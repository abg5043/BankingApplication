...
/**
 * Controller for screen where manager can create a new customer
 */
public class ManagerCreateUserController extends Controller {
  ...
  /**
   * Button that lets manager create a user
   */
  @FXML
  void createUserClicked(ActionEvent event) {
    ...
    //Check that the entered text is not blank and matches the expected formatting
    if (
        ssn.length() == 9 &&
            zip.length() == 5 &&
            username.length() > 0 &&
